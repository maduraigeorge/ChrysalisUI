  ...
  return (
    <SafeAreaView style={[styles.container, isDark && styles.containerDark]}>
      ...
      <Ionicons
        ...
        size={80}
        color="#6366f1"
        style={[styles.floatingIcon, { top: '6%', left: '4%' }]}
      />
      <Ionicons
        pointerEvents="none"
        name="school-outline"
        size={92}
        color="#f472b6"
        style={[styles.floatingIcon, { top: '10%', right: '4%' }]}
      />
      <Ionicons
        pointerEvents="none"
        name="create-outline"
        size={84}
        color="#fbbf24"
        style={[styles.floatingIcon, { bottom: '6%', right: '6%' }]}
      />
      ...
      <View style={[styles.centerBlock, styles.content]}>
        ...
        {/* extra breathing room after the title */}
      ...
      <View style={[styles.bottomArea, styles.content]}>
        ...
        <View style={styles.orRow}>
          ...
          <Text style={[styles.orText, isDark && styles.textDark]}>OR</Text>
          <View style={[styles.line, isDark && styles.lineDark]} />
        </View>
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.buttonBase,
        styles.primaryBtn,
        { opacity: pressed ? 0.93 : 1, transform: [{ scale: pressed ? 0.996 : 1 }] },
      ]}
    >
      <View style={[styles.iconWrapper, { backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.2)' }]}> 
        <Ionicons name={icon} size={24} color="#fff" />
      </View>
      <View>
        <Text style={[styles.btnTitle, { color: '#fff' }]}>{title}</Text>
        <Text style={[styles.btnSubtitle, { color: '#fff' }]}>{subtitle}</Text>
      </View>
    </Pressable>
  );
}
...
function SecondaryButton({ icon, title, subtitle, onPress }: ButtonProps) {
  ...
  return (
    ...
    >
      <View style={[styles.iconWrapper, { backgroundColor: isDark ? 'rgba(244,114,182,0.08)' : '#fce7f3' }]}>
        <Ionicons name={icon} size={24} color={isDark ? '#f9a0c8' : '#f472b6'} />
      </View>
      <View>

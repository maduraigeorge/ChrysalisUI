...
export default function WelcomeScreen() {
  ...
  // Sticky headers disabled — section titles now scroll with content
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.card, { paddingHorizontal: Math.round(containerPadding * 0.6), maxWidth: maxCardWidth, alignSelf: 'center' }]}>
        {/* Dashboard sections */}
        {activeTab === 'Activities' ? (
                      ...
                      {isOpen && (
                        ...
                        {sub.activities.map((a, i) => {
                            ...
                            if (parsed) {
                              ...
                              if (diff < 0) { dueLabel = `Overdue: ${formatDateShort(parsed)}`; dueColor = '#ef4444'; }
                              else if (diff === 0) { dueLabel = 'Due Today'; dueColor = '#f97316'; }
                              else if (diff === 1) { dueLabel = 'Due Tomorrow'; dueColor = '#f59e0b'; }
                              else if (diff <= 7) { dueLabel = `Due in ${diff} days`; dueColor = '#94A3B8'; }
                              else { dueLabel = `Due: ${formatDateShort(parsed)}`; dueColor = '#94A3B8'; }
                            }
